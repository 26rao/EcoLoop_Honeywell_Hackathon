...
from ecoloop.policy.fixed_schedule import RuleSetbackPolicy
...
def run_real_timer_simulation():
    print("==================================================")
    print("EcoLoop Benchmark — REAL EnergyPlus Programmable Timer Execution")
    print("==================================================")
    ...
    out_dir = str(project_root / "logs" / "real_timer_ep_out")
    log_path = project_root / "logs" / "real_timer_event_log.jsonl"

    policy = RuleSetbackPolicy()
    ...
    def callback_function(s):
        ...
        heat_rate = api.exchange.get_variable_value(s, handles["heat_rate"])
        cool_rate = api.exchange.get_variable_value(s, handles["cool_rate"])

        day = api.exchange.day_of_month(s)
        hour = api.exchange.current_time(s)
        sim_dt = datetime(2015, 7, day if day > 0 else 7, int(hour) % 24, 0, tzinfo=timezone.utc)
        hour_index = int(hour)

        occ_frac = 1.0 if 8 <= hour < 18 else 0.0

        readings = {
            "zone_temps_c": {"MainZone": round(zone_temp, 2)},
            "pmv": round(pmv_val, 2),
            "occupancy_fraction": occ_frac,
            "heating_energy_rate_w": max(0.0, round(heat_rate, 2)),
            "cooling_energy_rate_w": max(0.0, round(cool_rate, 2)),
            "outdoor_temp_c": 22.0,
            "hour_index": hour_index
        }

        heat_sp, cool_sp = controller.process_tick(sim_dt, readings, timestep_minutes=15)
    ...
    print("Executing real Programmable Timer closed-loop EnergyPlus simulation...")
    exit_code = api.runtime.run_energyplus(state_arg, cmd_args)
    elapsed = time.time() - start_time

    print("\n==================================================")
    print("Real EnergyPlus Programmable Timer Run Complete!")
